...
\section{import}
%The specification of which data to use in the program is very important because we want to build a generic program. The program uses XML files to do this. In the XML file the data file info, columns and the start and end of the data can be specified.\\
%To specify a File, first, a file name is needed. After that the path to the source folder is needed. Then the type of file and delimiter are specified. When the file specification is completed, the contents can be extracted. For this we need to know where the actual data starts and ends. This info is given by the amount of lines between the data and start/end of file. To recognise the data, each column gets a name and a type. When each column is specified, the file is read and a DataModel is created. 
...
% To be generic our program uses XML files to specify where the files are, what columns they contain and where the file starts and ends. This XML file can be loaded from the file system or be created inside our program using the \textit{XML-wizard}. 
...
%TODO reference to blackboard document "Exploratory Sequential Data Analysis: Foundations" by C. Fisher of the MAYA Design Group
