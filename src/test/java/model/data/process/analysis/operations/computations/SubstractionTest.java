...
public class SubstractionTest {
	...
	public void testIntFloatFail() throws Exception {
		Substraction check = new Substraction(new ConstantDescriber<>(new IntValue(123)), new ConstantDescriber<>(new FloatValue(456.567f)));
		check.compute(null);
	}

	@Test(expected = UnsupportedOperationException.class)
	public void testFloatIntFail() throws Exception {
		Substraction check = new Substraction(new ConstantDescriber<>(new FloatValue(123)), new ConstantDescriber<>(new IntValue(456)));
		check.compute(null);
	}
}